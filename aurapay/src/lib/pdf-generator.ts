...
/**
 * Generate a full professional payslip PDF for a given entry.
 * Used by both the download and email-send routes.
 */
export async function generatePayslipPdf(
  entryId: string,
  clientCalc?: PayslipCalculation | null
): Promise<PdfResult> {
  ...
  // THERAPY: Read raw values directly from DB row by column index
  ...
  const clinicName = settings.get("clinic_name") || "AuraPay";
  const clinicWebsite = settings.get("clinic_website") || "";
  ...
  if (patients.length > 0) {
    ...
    if (hasRichData) {
      autoTable(doc, {
        ...
        theme: "striped",
        headStyles: { fillColor: [navy.r, navy.g, navy.b], textColor: [255, 255, 255], fontSize: 6.5, fontStyle: "bold", cellPadding: 2 },
        bodyStyles: { fontSize: 6.5, cellPadding: 1.5 },
        alternateRowStyles: { fillColor: [248, 250, 252] },
        styles: { textColor: [slate700.r, slate700.g, slate700.b], overflow: "ellipsize" },
        columnStyles: { 0: { cellWidth: 45 }, 1: { cellWidth: 20 }, 2: { cellWidth: 55 }, 3: { halign: "right", cellWidth: 30 } },
      ...
      autoTable(doc, {
        ...
        theme: "striped",
        headStyles: { fillColor: [navy.r, navy.g, navy.b], textColor: [255, 255, 255], fontSize: 7, fontStyle: "bold", cellPadding: 2.5 },
        bodyStyles: { fontSize: 7, cellPadding: 2 },
        alternateRowStyles: { fillColor: [248, 250, 252] },
        styles: { textColor: [slate700.r, slate700.g, slate700.b] },
        columnStyles: { 0: { cellWidth: 60 }, 1: { cellWidth: 22 }, 2: { halign: "right", cellWidth: 30 }, 3: { halign: "center", cellWidth: 20 } },
        margin: { left: 15, right: 15 },
      });
    }
  }
