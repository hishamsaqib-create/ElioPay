...
export async function POST(req: NextRequest) {
  ...
  try {
    await transporter.sendMail({
      from: `"AuraPay" <${settings.email_from || settings.smtp_user}>`,
      to: entry.dentist_email,
      subject: `Your Payslip - ${getMonthName(entry.month)} ${entry.year} | Aura Dental`,
      html: `
        <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto">
          <div style="background:#4263eb;padding:20px;border-radius:8px 8px 0 0">
            <h1 style="color:#fff;margin:0;font-size:24px">AuraPay</h1>
            <p style="color:#bac8ff;margin:5px 0 0">Aura Dental Clinic</p>
          </div>
          <div style="padding:25px;background:#fff;border:1px solid #e9ecef">
            ...
            <p style="font-size:13px;color:#868e96">This payslip was generated by AuraPay. If you have questions, please contact the practice manager.</p>
          </div>
          <div style="padding:15px;text-align:center;font-size:11px;color:#adb5bd">
            Powered by AuraPay | aurapay.cloud
          </div>
        </div>
      `,
