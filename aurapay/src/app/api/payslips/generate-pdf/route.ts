...
export async function GET(req: NextRequest) {
  ...
  for (const adj of calc.adjustments) {
    const prefix = adj.type === "deduction" ? "-" : "+";
    deductionsData.push([`${prefix} ${adj.description}`, formatCurrency(adj.amount)]);
  }
  ...
  // Private patients if any
  const patients = JSON.parse(entry.private_patients_json || "[]");
  if (patients.length > 0) {
    ...
    doc.autoTable({
      ...
      body: patients.map((p: { name: string; date: string; amount: number; finance: boolean }) => [
        p.name, p.date, formatCurrency(p.amount), p.finance ? "Yes" : "",
      ]),
      theme: "striped",
      headStyles: { fillColor: [66, 99, 235], fontSize: 8 },
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Generated by AuraPay | aurapay.cloud | Page ${i} of ${pageCount}`,
